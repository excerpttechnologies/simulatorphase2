...
export class TwoRobotChipProcess {
  ...
  _setState(newState) {
    ...
    if (oldState !== newState) {
      ...
      if (text && this.bonder?.narration?.isEnabled) {
        this.bonder.narration.speak(text, 'normal')
      }
      console.log(`[TwoRobot] State: ${oldState} → ${newState}`)
    }
  }
  ...
  async processChip(index) {
    ...
    this._setState('R1_MOVE_TO_CHIP')
    await this.moveRobot1ToChip(chip)

    this._setState('R1_PICK')
    this.attachToRobot1(chip)
    await this.moveRobot1Lift()

    this._setState('R1_MOVE_TO_FLIP')
    await this.moveRobot1ToFlip()

    this._setState('R1_FLIP')
    await this.flipChip180(chip)

    this._setState('R1_MOVE_TO_HANDOFF')
    await this.moveRobot1ToHandoff()

    this._setState('R1_HANDOFF_READY')

    this._setState('R2_MOVE_TO_HANDOFF')
    await this.moveRobot2ToHandoff()

    this._setState('R2_PICK')
    this.transferToRobot2(chip)
    await this.moveRobot2Lift()

    this._setState('R2_MOVE_TO_FLUX')
    await this.moveRobot2ToFlux()

    this._setState('R2_FLUX_DIP')
    await this.dipFlux(chip)

    this._setState('R2_MOVE_TO_BOARD')
    await this.moveRobot2ToBoard(index)

    this._setState('R2_PLACE')
    const target = this.getChipTarget(index)
  ...
  async runSixChips() {
    ...
    try {
      ...
      this._setState('SIX_CHIPS_COMPLETE')
