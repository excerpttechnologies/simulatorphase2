...
// ===== GLB-REMOVED (GLTFLoader import) - re-wire the new module here. See GLB_WIRING_CONTRACT.md =====
...
export class BonderController {
  ...
  // ===== GLB-REMOVED (BonderController.load - /flip_chip_bonder.glb) =====
  // Stubbed so callers that still `await load()` resolve safely instead of
  // fetching a GLB. `this.ready` stays false, so start() no-ops and update()
  // skips every node-dependent branch. The original body is preserved below.
  // Re-wire the new module here. See GLB_WIRING_CONTRACT.md
  async load() {
    this._log?.('GLB loading disabled - BonderController.load() is a stub')
    return { scene: null, clips: [], status: this.getStatus?.() ?? null }
  }

  // ── Original GLB load(), preserved verbatim for the replacement module ──
//   async _load_GLB_REMOVED(url = '/flip_chip_bonder.glb') {
    // const gltf = await new Promise((resolve, reject) => {
    //   new GLTFLoader().load(url, resolve, undefined, reject)
    // })
//     const gltf = { scene: null, animations: [] }
// 
