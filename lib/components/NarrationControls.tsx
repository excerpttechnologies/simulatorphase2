...
const NarrationControls: React.FC<Props> = ({ simRef }) => {
  ...
  useEffect(() => {
    const check = setInterval(() => {
      ...
      // Also check for bonder controller narration
      if (simRef.current?.bonderController?.narration) {
        setEnabled(simRef.current.bonderController.narration.isEnabled());
        clearInterval(check);
      }
    }, 100);
  ...
  useEffect(() => {
    ...
    const onSimReset = () => {
      ...
      if (sim?.bonderController?.narration) {
        try { sim.bonderController.narration.stop?.(); } catch (e) {}
        try { sim.bonderController.narration.reset?.(); } catch (e) {}
        try { setEnabled(sim.bonderController.narration.isEnabled()); } catch (e) {}
      }
  ...
  const handleToggle = () => {
    ...
    simRef.current?.bonderController?.setNarrationEnabled?.(newState);
  };
  
  const handleVolume = (v: number) => {
    setVolume(v);
    simRef.current?.narration?.setVolume(v);
    simRef.current?.bonderController?.setNarrationVolume?.(v);
  };

  const handleRate = (r: number) => {
    setRate(r);
    simRef.current?.narration?.setRate(r);
    simRef.current?.bonderController?.setNarrationRate?.(r);
  };
        ...
        <div style={{
          ...
          <button
            onClick={() => {
              ...
              simRef.current?.bonderController?.narration?.speak(
                'Testing bonder narration. This is your avatar voice.',
                'high'
              );
            }}
            style={{
